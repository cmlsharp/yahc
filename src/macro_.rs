...
macro_rules! generate_hashcons {
    ($Data:ty) => {
        ...
        struct NodeDataRef<'a, Q: Borrow<[Node]>>(&'a $Data, &'a Q);
        ...
        pub struct Table {}
        impl $crate::HasTable for $Data {
            type TableType = Table;
        }

        impl $crate::Table<$Data> for Table {
            type Node = Node;
            type Weak = Weak;

            fn with_debug_info(f: impl FnOnce(&dyn std::fmt::Debug)) {
                MANAGER.with(|man| f(man))
            }

            #[allow(dead_code)]
            fn create(data: $Data, children: impl std::convert::Into<Box<[Node]>>) -> Node {
                MANAGER.with(|man| man.create(data, children))
            }
            ...
            fn for_each(mut f: impl FnMut(&$Data, &[Self::Node])) {
                MANAGER.with(|man| man.table.borrow().keys().for_each(|n| f(&n.data, &n.cs)));
            }
        ...
        impl Manager {
            fn create(&self, data: $Data, children: impl std::convert::Into<Box<[Node]>>) -> Node {
                ...
                if !std::thread::panicking() {
                    ...
                    while let Some(t) = to_collect.pop() {
                        ...
                        let data = Rc::try_unwrap(strong_data).unwrap_or_else(|d| {
                            panic!("too many refs to {:?}", NodeListShallowDebug(&d.cs))
                        });
